...
class PerformanceProfiler:
    ...
    def analyze_distribution(self) -> Dict:
        ...
        for metric in self.metrics:
            ...
            node_metrics[metric.node_id]["execution_times"].append(
        ...
        load_metrics = {
            ...
            "avg_tests_per_node": np.mean(
                [m["total_tests"] for m in node_metrics.values()]
            ),
            "std_tests_per_node": np.std(
                [m["total_tests"] for m in node_metrics.values()]
            ),
            "avg_queue_time": np.mean(
                [np.mean(m["queue_times"]) for m in node_metrics.values()]
            ),
            "avg_execution_time": np.mean(
                [np.mean(m["execution_times"]) for m in node_metrics.values()]
            ),
        }
    ...
    def optimize_distribution(self, current_strategy: str) -> Dict:
        ...
        recommendations = {
            ...
            "distribution_efficiency": distribution_metrics.get(
                "std_tests_per_node", 0
            ),
        ...
        self.optimization_history.append(
            {"timestamp": datetime.now(), "recommendations": recommendations}
        )
