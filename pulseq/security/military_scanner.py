...
class MilitaryScanner(EnterpriseScanner):
            ...
            for port, service in open_ports.items():
                if port in [21, 23, 80, 443]:  # Common services
                    if not self._is_service_required(service):
                        exposed['high'].append(f'Unnecessary service exposed: {service} on port {port}')
        ...
        try:
            ...
            for service in services:
                try:
                    ...
                    version = subprocess.check_output(['systemctl', 'show', service, '--property=Version']).decode()
        ...
        try:
            ...
            for directory in critical_dirs:
                ...
                if mode & 0o777 != 0o755:  # Should be rwxr-xr-x
                    misconfigs['critical'].append(f'Incorrect permissions on {directory}')
                    ...
                    if os.path.exists(config_file):
                        with open(config_file, 'r') as f:
                            ...
                            if 'PermitRootLogin yes' in config:
                                misconfigs['critical'].append(f'Insecure {service} configuration')
                except Exception:
                    continue
    ...
    def _check_access_control(self) -> Dict[str, bool]:
        ...
        try:
            return {
                ...
                'separation_of_duties': self.config.get('enforce_separation_of_duties', False),
                'session_management': self.config.get('enforce_session_management', False),
    ...
    def _check_audit_logging(self) -> Dict[str, bool]:
        ...
        try:
            return {
                'comprehensive_logging': self.config.get('enable_comprehensive_logging', False),
    ...
    def _check_identification(self) -> Dict[str, bool]:
        ...
        try:
            return {
                'unique_identification': self.config.get('enforce_unique_identification', False),
                'authenticator_management': self.config.get('manage_authenticators', False),
    ...
    def _check_system_communications(self) -> Dict[str, bool]:
        ...
        try:
            return {
                'boundary_protection': self.config.get('enforce_boundary_protection', False),
                'transmission_confidentiality': self.config.get('enforce_transmission_confidentiality', False),
                'network_disconnect': self.config.get('enforce_network_disconnect', False),
