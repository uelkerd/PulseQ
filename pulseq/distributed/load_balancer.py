...
class OptimizedLoadBalancer:
    ...
    def update_node_load(self, node_id: str, load_info: Dict):
        ...
        if node_id in self.nodes:
            ...
            node.network_latency = load_info.get(
                "network_latency", node.network_latency
            )
            node.active_connections = load_info.get(
    ...
    def _update_weights(self):
        ...
        load_weights = {
            node_id: 1.0
            / (node.current_load + 0.1)  # Add small constant to avoid division by zero
            for node_id, node in self.nodes.items()
        }
        ...
        if total_weight > 0:
            self.weights = {
                node_id: (capability_weights[node_id] * load_weights[node_id])
                / total_weight
    ...
    def _least_load(self, nodes: List[str]) -> str:
        ...
        return min(nodes, key=lambda node_id: self.nodes[node_id].current_load)
    ...
    def _least_connections(self, nodes: List[str]) -> str:
        ...
        return min(nodes, key=lambda node_id: self.nodes[node_id].active_connections)
    ...
    def _predictive_load(self, nodes: List[str]) -> str:
        ...
        return min(nodes, key=lambda node_id: self.nodes[node_id].predicted_load)
        ...
        for rec in recommendations["recommendations"]:
            if rec["type"] == "load_balancing" and rec["priority"] == "high":
                if rec["suggestion"] == "least_connections":
                    ...
                    self.logger.info(
                        "Switched to least-connections strategy based on optimization"
                    )
                elif rec["suggestion"] == "response_time":
                    self.strategy = "response_time"
                    self.logger.info(
                        "Switched to response-time strategy based on optimization"
                    )
                elif rec["suggestion"] == "predictive":
                    self.strategy = "predictive"
                    self.logger.info(
                        "Switched to predictive strategy based on optimization"
                    )
                else:
                    self.strategy = "least_load"
                    self.logger.info(
                        "Switched to least-load strategy based on optimization"
                    )
            elif rec["type"] == "distribution" and rec["priority"] == "high":
                self.strategy = "weighted_round_robin"
                self.logger.info(
                    "Switched to weighted round-robin strategy based on optimization"
                )
    ...
    def get_node_stats(self) -> Dict:
        ...
        return {
            node_id: {
                ...
                "active_connections": node.active_connections,
                "avg_response_time": (
                    statistics.mean(node.response_times) if node.response_times else 0
                ),
    ...
    def _update_strategy_metrics(self):
        ...
        avg_selection_time = (
            statistics.mean(self.selection_times) if self.selection_times else 0
        )
        ...
        success_rate = (
            successful_selections / total_selections if total_selections > 0 else 1.0
        )

        # Calculate load balance score
        node_loads = [node.current_load for node in self.nodes.values()]
        load_balance_score = (
            1.0 - statistics.stdev(node_loads) if len(node_loads) > 1 else 1.0
        )

        # Calculate resource efficiency
        resource_efficiencies = [
            metrics.resource_utilization
            for metrics_list in self.node_metrics_history.values()
            for metrics in metrics_list[-10:]  # Last 10 metrics
        ]
        resource_efficiency = (
            statistics.mean(resource_efficiencies) if resource_efficiencies else 0
        )
    ...
    def get_detailed_metrics(self) -> Dict:
        ...
        if (
            datetime.now() - self.last_metrics_update
        ...
        latest_strategy_metrics = (
            self.strategy_metrics_history[-1] if self.strategy_metrics_history else None
        )
            ...
            "strategy": (
                {
                    ...
                    "avg_selection_time": (
                        latest_strategy_metrics.avg_selection_time
                        if latest_strategy_metrics
                        else 0
                    ),
                    "success_rate": (
                        latest_strategy_metrics.success_rate
                        if latest_strategy_metrics
                        else 1.0
                    ),
                    "load_balance_score": (
                        latest_strategy_metrics.load_balance_score
                        if latest_strategy_metrics
                        else 1.0
                    ),
                    "resource_efficiency": (
                        latest_strategy_metrics.resource_efficiency
                        if latest_strategy_metrics
                        else 0
                    ),
                    "error_rate": (
                        latest_strategy_metrics.error_rate
                        if latest_strategy_metrics
                        else 0
                    ),
                    "transition_count": (
                        latest_strategy_metrics.transition_count
                        if latest_strategy_metrics
                        else 0
                    ),
                    "last_transition_time": (
                        latest_strategy_metrics.last_transition_time
                        if latest_strategy_metrics
                        else None
                    ),
                }
