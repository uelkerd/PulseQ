...
class ResultsAggregator:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "start_time": min(r.timestamp for r in self.results.values()) if self.results else None,
            "end_time": max(r.timestamp for r in self.results.values()) if self.results else None
        }
            ...
            if result.metrics:
                for metric_name, value in result.metrics.items():
                    ...
                    metrics_summary[metric_name]["min"] = min(
                        metrics_summary[metric_name]["min"],
                        value
                    )
                    metrics_summary[metric_name]["max"] = max(
                        metrics_summary[metric_name]["max"],
                        value
                    )
