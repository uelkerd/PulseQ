...
from .test_runner import TestNode, DistributedTestRunner
...
class TestScheduler:
    ...
    def get_node_load(self, node_id: str) -> int:
        ...
        """
        return sum(
            1 for test_info in self.scheduled_tests.values()
            if test_info["status"] == "running" and test_info["assigned_node"] == node_id
        )
