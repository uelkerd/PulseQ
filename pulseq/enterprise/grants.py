...
class GrantManager:
    ...
    async def create_grant_proposal(self, grant_type: GrantType,
    ...
    async def get_publication_metrics(self, grant_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_impact_factor": sum(
                p.impact_factor for p in publications if p.impact_factor
            ) / len(publications) if publications else 0,
    ...
    async def update_milestone_status(self, milestone_id: str,
                                    status: str,
                                    completion_date: Optional[datetime] = None) -> None:
