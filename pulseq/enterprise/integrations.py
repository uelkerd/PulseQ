...
class ComplianceIntegrator:
    ...
    async def _start_sync_task(self, system: ExternalSystem) -> None:
        ...
        self.sync_tasks[system.name] = asyncio.create_task(self._sync_system(system))
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    raise Exception(
                        f"Failed to sync with {system.name}: {response.status}"
                    )

    async def _process_sync_data(
        self, system: ExternalSystem, data: Dict[str, Any]
    ) -> None:
    ...
    async def sync_package(self, package: CompliancePackage) -> None:
        ...
        relevant_systems = [
            ...
            if any(
                framework in system.supported_frameworks
    ...
    async def _sync_package_with_system(
        self, package: CompliancePackage, system: ExternalSystem
    ) -> None:
    ...
    async def get_system_status(self, system_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_frameworks": [f.value for f in system.supported_frameworks],
            "sync_task_running": system.name in self.sync_tasks,
        }

    async def validate_requirement(
        self, requirement: ComplianceRequirement, system_name: str
    ) -> Dict[str, Any]:
    ...
    async def get_system_recommendations(
        self, package: CompliancePackage, system_name: str
    ) -> List[Dict[str, Any]]:
    ...
    async def export_compliance_data(
        self, package: CompliancePackage, format: str = "json"
    ) -> Dict[str, Any]:
