...
class PortalManager:
    ...
    async def update_subscription(self,
        ...
        try:
            ...
            # Update subscription
            updated = self.billing.update_subscription(
                subscription.id,
                new_plan
            )
            ...
            self.license.update_license_features(
                customer_id,
                self._get_features_for_plan(new_plan)
            )
    ...
    async def get_billing_history(self,
                                customer_id: str) -> Dict[str, Any]:
        """Get customer billing history"""
        try:
            invoices = self.billing.stripe.Invoice.list(
                customer=customer_id
            )
    ...
    async def get_usage_report(self,
        ...
        """Generate usage report for customer"""
        try:
            report = self.usage.generate_report(
                customer_id,
                start_date,
                end_date
            )
