...
from pulseq.config.config_manager import (
    ConfigManager,
    EdgeCaseConfig,
    LoadBalancerConfig,
...
def config_file(tmp_path):
    ...
    config = {
        ...
        "workload_patterns": {
            "basic": {"requests_per_second": 100, "request_size": 1024, "duration": 60},
            "ecommerce": {
        ...
        "edge_cases": {"cpu_saturation": {"cpu_usage": 0.95, "duration": 30}},
        "metrics": {
...
def test_validate_workload_pattern(config_file):
    ...
    pattern = WorkloadPatternConfig(
        requests_per_second=100, request_size=1024, duration=60
    )
...
def test_validate_edge_case(config_file):
    ...
    case = EdgeCaseConfig(cpu_usage=0.5, duration=30)
