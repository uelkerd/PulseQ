...
def mock_stripe():
    with patch('stripe.Subscription.create') as mock_sub_create, \
         ...
         patch('stripe.Subscription.delete') as mock_sub_delete, \
         patch('stripe.Invoice.list') as mock_invoice_list, \
         patch('stripe.Invoice.upcoming') as mock_invoice_upcoming, \
         patch('stripe.PaymentMethod.create') as mock_pm_create, \
         patch('stripe.PaymentMethod.modify') as mock_pm_modify, \
         patch('stripe.PaymentMethod.detach') as mock_pm_detach, \
         patch('stripe.SubscriptionItem.create_usage_record') as mock_usage_record, \
         patch('stripe.SubscriptionItem.retrieve_usage_record_summary') as mock_usage_summary, \
         patch('stripe.Webhook.construct_event') as mock_webhook, \
         patch('stripe.PaymentIntent.create') as mock_pi_create, \
         patch('stripe.PaymentIntent.confirm') as mock_pi_confirm, \
         patch('stripe.Customer.create') as mock_customer_create, \
         patch('stripe.Customer.retrieve') as mock_customer_retrieve, \
...
async def test_report_usage(billing_manager, mock_stripe):
    ...
    # Test
    result = await billing_manager.report_usage(
        'si_test_123',
        100
    )
