...
from pulseq.distributed.load_balancer import OptimizedLoadBalancer, NodeLoad
...
async def test_dynamic_weight_adjustment(load_balancer, sample_nodes):
    ...
    assert updated_weights["node1"] < initial_weights["node1"]  # Weight should decrease with higher load
...
async def test_capability_based_weights(load_balancer, sample_nodes):
    ...
    assert weights["node3"] > weights["node2"]  # node3 has same capabilities as node2 but Linux OS
...
async def test_load_distribution_fairness(load_balancer, sample_nodes):
    ...
    selection_ratios = {
        node_id: count / total_selections
        for node_id, count in node_selections.items()
    }
