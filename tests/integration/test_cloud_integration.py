...
def test_create_test_environment(cloud_manager, aws_config, mock_aws_provider):
    ...
    result = cloud_manager.create_test_environment('test', {
        'image': 'test-image',
        'cpu': 256,
        'memory': 512
    })
...
def test_destroy_test_environment(cloud_manager, aws_config, mock_aws_provider):
    ...
    cloud_manager.destroy_test_environment('arn:aws:ecs:region:account:cluster/pulseq-test')
...
def test_get_environment_status(cloud_manager, aws_config, mock_aws_provider):
    ...
    status = cloud_manager.get_environment_status('arn:aws:ecs:region:account:cluster/pulseq-test')
...
def test_get_cost_estimate(cloud_manager, aws_config, mock_aws_provider):
    ...
    cost = cloud_manager.get_cost_estimate('arn:aws:ecs:region:account:cluster/pulseq-test')
...
def test_create_gcp_test_environment(gcp_config, mock_gcp_cluster):
    ...
    with patch('google.cloud.container_v1.ClusterManagerClient') as mock_client:
        ...
        manager.initialize_provider('gcp', gcp_config)
        result = manager.create_test_environment('test-cluster', {
            'node_count': 3,
...
def test_scale_gcp_environment(gcp_config):
    ...
    with patch('google.cloud.container_v1.ClusterManagerClient') as mock_client:
        ...
        manager.initialize_provider('gcp', gcp_config)
        manager.scale_environment('pulseq-test-cluster', {
            'node_count': 5
        })
...
def test_get_gcp_cost_estimate(gcp_config):
    manager = CloudManager()
    with patch('google.cloud.container_v1.ClusterManagerClient') as mock_client:
        mock_client.return_value.get_cluster.return_value = Mock(
            node_count=3
        )
...
def test_initialize_azure_provider(azure_config):
    ...
    with patch('azure.identity.DefaultAzureCredential') as mock_credential, \
         patch('azure.mgmt.containerservice.ContainerServiceClient') as mock_container_client, \
...
def test_destroy_azure_test_environment(azure_config):
    ...
    with patch('azure.identity.DefaultAzureCredential') as mock_credential, \
        ...
        manager.initialize_provider('azure', azure_config)
        manager.destroy_test_environment('/subscriptions/test-subscription-id/resourceGroups/test-resource-group/providers/Microsoft.ContainerService/managedClusters/pulseq-test-cluster')
...
def test_scale_azure_environment(azure_config, mock_azure_cluster):
    ...
    with patch('azure.identity.DefaultAzureCredential') as mock_credential, \
         patch('azure.mgmt.containerservice.ContainerServiceClient') as mock_container_client:
        mock_container_client.return_value.managed_clusters.get.return_value = mock_azure_cluster
        mock_container_client.return_value.managed_clusters.begin_create_or_update.return_value = Mock(
            result=lambda: mock_azure_cluster
        )
        
        manager.initialize_provider('azure', azure_config)
        manager.scale_environment('/subscriptions/test-subscription-id/resourceGroups/test-resource-group/providers/Microsoft.ContainerService/managedClusters/pulseq-test-cluster', {
            'node_count': 5
        })
...
def test_get_azure_environment_status(azure_config, mock_azure_cluster):
    ...
    with patch('azure.identity.DefaultAzureCredential') as mock_credential, \
        ...
        manager.initialize_provider('azure', azure_config)
        status = manager.get_environment_status('/subscriptions/test-subscription-id/resourceGroups/test-resource-group/providers/Microsoft.ContainerService/managedClusters/pulseq-test-cluster')
...
def test_list_azure_environments(azure_config, mock_azure_cluster):
    ...
    with patch('azure.identity.DefaultAzureCredential') as mock_credential, \
         patch('azure.mgmt.containerservice.ContainerServiceClient') as mock_container_client:
        mock_container_client.return_value.managed_clusters.list_by_resource_group.return_value = [mock_azure_cluster]
...
def test_get_azure_cost_estimate(azure_config, mock_azure_cluster):
    ...
    with patch('azure.identity.DefaultAzureCredential') as mock_credential, \
        ...
        manager.initialize_provider('azure', azure_config)
        cost = manager.get_cost_estimate('/subscriptions/test-subscription-id/resourceGroups/test-resource-group/providers/Microsoft.ContainerService/managedClusters/pulseq-test-cluster')
