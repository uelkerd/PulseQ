...
async def test_priority_based_execution(test_environment):
    ...
    tests = [
        ...
        for i, priority in enumerate(["critical"] * 5 + ["high"] * 10 + ["medium"] * 15 + ["low"] * 20)
    ]
    ...
    critical_results = [r for r in results if r.test_id.startswith("test_") and int(r.test_id.split("_")[1]) < 5]
    high_results = [r for r in results if r.test_id.startswith("test_") and 5 <= int(r.test_id.split("_")[1]) < 15]
